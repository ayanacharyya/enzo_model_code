...
def func(i):
    ...
    t_ch = np.sqrt(4*np.pi*rho_0*r_0**k_rho*c*r_ch**(4-k_rho)/((3-k_rho)*f_trap*psi*eps*Q_H0[i]))
    tau = age_SI/t_ch
    ...
    s = str(i)+' '+str(x[i])+' '+str(y[i])+' '+str(float(r_stall/3.06e16))+' '+str(float(rII/3.06e16))+' '\
    +str(float(r/3.06e16))+' '+str(Q_H0[i])+' '+str(nII)+' '+str(IP)+' '+str(P_over_k)+' '+str(float(age[i]))+' '\
...
if __name__ == '__main__':
    ...
    merged = 0
    #----------------------Reading starburst-------------------------------------------
    lum = open('/Users/acharyya/SB99-v8-02/output/starburst03/starburst03.quanta', 'r') ##previously done with different SB output
    ...
    for line in list:
        if float(line.split()[5]) >= 0.1:
            ...
            Q_H0.append(float(line.split()[4])*10**(f(math.log10(float(line.split()[5])))-6))# '6' bcz starburst was run for mass of 1M Msun
    ...
    print str(stalled)+' HII regions have stalled expansion which is '+str(stalled*100./len(x))+' % of the total.'
    print str(merged)+' HII regions grown beyond own cell which is '+str(merged*100./len(x))+' % of the total.'
