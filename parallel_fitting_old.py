...
if __name__ == '__main__':
    ...
    for k in range(core_start, core_end):
        ...
        if np.array(flam).any():
            cont = po.fitcont(wave, flam, args, logbook) #cont in units ergs/s/A/pc^2
...
str(np.max(flam))+','+str(np.min(flam))+'\n', args)
        ...
        mapcube_local[i,j,:], errorcube_local[i,j,:] = po.fit_all_lines(args, logbook, wave, flam, cont, i, j, z=0., z_err=0.0001)
