...
master_table = master_table[
    master_table['fixed_noise'].isin([3, 8, 10])]  ##to forcibly exclude/include specific "fixed_noise" cases
master_table = master_table[master_table['snr_cut'].isin([0, 3, 5])]  ##to forcibly exclude/include specific "snr" cases
master_table = master_table[
    master_table['logOHgrad'].isin([-0.1, -0.05, -0.01])]  ##to forcibly exclude/include specific "Zgrad" cases
if nonoise_exists:
...
for (row, logOHgrad_choice) in enumerate(logOHgrad_choice_arr):
    ...
    if highsnr_exists:
        highsnr_y_arr = (highsnr_master_table2[ycol1].values - highsnr_master_table2['logOHgrad'].values) * 100. / \
                        highsnr_master_table2['logOHgrad'].values
    ...
    for (col, snr_cut_choice) in enumerate(snr_cut_choice_arr):
        ...
        for (ind, fsnr) in enumerate(np.unique(master_table3[snr_mode])):
            ...
            ax1.scatter(table2[xcol].values, y_arr, c=col_ar[ind], linewidth=0,
                        label='SNR inp=' + str(fsnr) + ', out=' + str('%.2F +/- %.2F' % (
                            np.mean(table2['snr_measured'].values), np.mean(table3['snr_measured'].values))))
        ...
        try:
            ...
            pass
        ax1.set_ylim(ylim1)
        ax1.text(ax1.get_xlim()[-1] * 0.99, np.diff(ax1.get_ylim()) * 0.08, 'steeper', color='k', fontsize=10,
                 ha='right')
        ax1.text(ax1.get_xlim()[-1] * 0.99, -np.diff(ax1.get_ylim()) * 0.08, 'shallower', color='k', fontsize=10,
                 ha='right')
