...
def gauss(w, f, w0, f0, v, vz):
    ...
    return f
#-------------------------------------------------------------------------------------------
def bpt_pixelwise(s, Om, res, res_phys, saveplot=False, smooth=False, ker = None, parm=None, addnoise=False, maketheory=False):
    info=''
    ...
    if smooth:
        mapn2, info = smoothmap(mapn2, res, res_phys, parm=parm, ker=ker, maskzero=True, addnoise = addnoise, maketheory = maketheory)
        mapha, info = smoothmap(mapha, res, res_phys, parm=parm, ker=ker, maskzero=True, addnoise = addnoise, maketheory = maketheory)
        mapo3, info = smoothmap(mapo3, res, res_phys, parm=parm, ker=ker, maskzero=True, addnoise = addnoise, maketheory = maketheory)
        maphb, info = smoothmap(maphb, res, res_phys, parm=parm, ker=ker, maskzero=True, addnoise = addnoise, maketheory = maketheory)
        res = np.round(galsize/np.shape(mapn2)[0],2) #kpc
        g = np.shape(mapn2)[0]
    b=np.linspace(-g/2+1,g/2,g)
    d = np.sqrt(b[:,None]**2+b**2)*res
    print 'debug', g, res, np.min(b), np.max(d) #
...
#-------------------------------------------------------------------------------------------
def metallicity(s, Om, res, res_phys, getmap=False, hide=False, cmin=None, cmax=None, saveplot=False, smooth=False, ker = None, parm=None, addnoise=False, maketheory=False):
    info=''
    g,x,y = calcpos(s, galsize, res)
    r1 = np.log10(s['NII6584']/s['SII6730'])#
    r2 = np.log10(s['NII6584']/s['H6562'])#
    m = 8.77 + r1 + 0.264*r2#
    m = 10**(m-8.69)#
    print m #
    print np.median(m), np.mean(m)#
    mapn2 = make2Dmap(s['NII6584']/((res*1e3)**2), x, y, g, res) #ergs/s/pc^2
    mapo2 = make2Dmap(s['OII3727']/((res*1e3)**2), x, y, g, res) #ergs/s/pc^2
    maps2 = make2Dmap(s['SII6730']/((res*1e3)**2), x, y, g, res) #ergs/s/pc^2
    mapha = make2Dmap(s['H6562']/((res*1e3)**2), x, y, g, res) #ergs/s/pc^2
    if smooth:
        mapn2, info = smoothmap(mapn2, res, res_phys, parm=parm, ker=ker, addnoise = addnoise, maketheory = maketheory)
        mapo2, info = smoothmap(mapo2, res, res_phys, parm=parm, ker=ker, addnoise = addnoise, maketheory = maketheory)
        maps2, info = smoothmap(maps2, res, res_phys, parm=parm, ker=ker, addnoise = addnoise, maketheory = maketheory)
        mapha, info = smoothmap(mapha, res, res_phys, parm=parm, ker=ker, addnoise = addnoise, maketheory = maketheory)
        res = np.round(galsize/np.shape(mapn2)[0],2) #kpc
        g = np.shape(mapn2)[0]
    b=np.linspace(-g/2,g/2,g)
    d = np.sqrt(b[:,None]**2+b**2)*res #dist in kpc
    
    t = title(fn)+' Metallicity gradient \n\
for Omega = '+str(Om)+', resolution = '+str(res)+' kpc'+info
    '''
    #--------From Kewley 2001------------------
    ...
    '''
    #-------From Dopita 2016------------------------------
    logOHsol = 8.69 #log(O/H)+12 value used for Solar metallicity in MAPPINGS-V, Asplund 2009
    log_ratio = np.log10(np.divide(mapn2,maps2)) + 0.264*np.log10(np.divide(mapn2,mapha))
    met_map = 8.77 + log_ratio #+ 0.45*(log_ratio + 0.3)**5
    t += '_newdiag'
    
    #---------------------------------------------------
    met_map = 10**(met_map-logOHsol) #converting to Z from log(O/H) + 12
    cbarlab = 'Z/Z_sol'
    if getmap:
        map = plotmap(met_map, t, 'Metallicity', cbarlab, galsize, res, cmin = cmin, cmax =cmax, hide = hide, saveplot=saveplot, islog=False)
    else:
        fig = plt.figure(figsize=(8,6))
        fig.subplots_adjust(hspace=0.7, top=0.85, bottom=0.1, left=0.1, right=0.95)
        ax = plt.subplot(111)
        plt.scatter(d.flatten(),met_map.flatten(), s=1, lw=0, label='pixels')
        plt.axhline(10**(8.6-logOHsol),c='k',lw=0.5, label='trust') #line below which metallicity values should not be trusted
        plt.axhline(1,c='r',linestyle='--',label='Zsol') #line for solar metallicity
        ...
        if saveplot:
            fig.savefig(path+t+'.png')
#-------------------------------------------------------------------------------------------
def emissionmap(s, Om, res, res_phys, line, saveplot=False, smooth=False, off=None, ker = None, parm=None, hide=False, addnoise = False, cmin=None, cmax=None, maketheory=False):
    info=''
    g,x,y = calcpos(s, galsize, res)
    flux = s[line]/((res*1e3)**2) #ergs/s/pc^2
    map = make2Dmap(flux, x, y, g, res)
    if smooth:
        map, info = smoothmap(map, res, res_phys, parm=parm, ker=ker, addnoise = addnoise, maketheory = maketheory)
        res = np.round(galsize/np.shape(map)[0],2) #kpc
    t = title(fn)+line+' map for Omega = '+str(Om)+', resolution = '+str(res)+' kpc'+info
    ...
    return map
#-------------------------------------------------------------------------------------------
def SFRmaps(s, Om, reso, res_phys, getmap=True, saveplot=False, smooth=False, ker = None, off=None, parm=None, hide=False, addnoise = False, cmin=None, cmax=None, maketheory=False):
    flux = s['H6562'] #ergs/s
    ...
    for i, res in enumerate(reso):
        ...
        info = ''
        if smooth:
            SFRmapQ0, info = smoothmap(SFRmapQ0, res, res_phys, parm=parm, ker=ker, maskzero=True, addnoise = addnoise, maketheory = maketheory, units_in_photon=True)
            SFRmapHa, info = smoothmap(SFRmapHa, res, res_phys, parm=parm, ker=ker, maskzero=True, addnoise = addnoise, maketheory = maketheory)
            SFRmap_real = rebin_old(SFRmap_real,np.shape(SFRmapHa))
            res = np.round(galsize/np.shape(SFRmapHa)[0],2) #kpc
        else: 
...
def readSB(wmin, wmax):
    ...
    return funcar
#-------------------------------------------------------------------------------------------
def spec(s, Om, res, res_phys, col, wmin = None, wmax = None, changeunits= False, off=None, getcube=False, X=None, Y=None, spec_smear=False, plotspec=False, plotintegmap=False, addnoise = False, savecube=False, saveplot=False, smooth=False, ker = None, parm=None, hide=False, cmin=None, cmax=None, maketheory=False):
    global fitsname
    ...
    if smooth and not plotintegmap: 
        ppv, info = smoothcube(ppv, res, res_phys, parm=parm, addnoise = addnoise, maketheory=maketheory, changeunits=changeunits, info=info) #spatially smooth the PPV using certain parameter set
    ...
    if plotintegmap:
        ...
        if smooth: 
            map, info = smoothmap(map, res, res_phys, parm=parm, ker=ker, addnoise = addnoise, maketheory=maketheory, changeunits=changeunits, info=info) #should not need this after smoothcube is working
            res = np.round(galsize/np.shape(map)[0],2) #kpc
        t = title(fn)+line+' map for Omega = '+str(Om)+', res = '+str(res)+' kpc'+info
        map = plotmap(map, t, line, cbarlab, galsize, res, cmin = cmin, cmax =cmax, hide = hide, saveplot=saveplot, maketheory=maketheory)            
...
def make2Dmap(data, xi, yi, gridsize, res, domean=False, islog=False):
    ...
    return map 
#-------------------------------------------------------------------------------------------
def smoothmap(map, res, res_phys, parm=None, ker='moff', maskzero=False, addnoise = False, maketheory=False, info='', silent = False, changeunits=False, units_in_photon=False):
    if parm is None:
    ...
    if '_smeared_' not in info: info += '\n_smeared_'+ker+'_parm'+str((sig,pow,size))    
    ...
    return map, info
#-------------------------------------------------------------------------------------------
def smoothcube(cube, res, res_phys, addnoise = False, maketheory = False, parm=None, info='', changeunits=False):
    zlim = np.shape(cube)[2]
    for k in range(zlim):
        map, info = smoothmap(cube[:,:,k], res, res_phys, addnoise = addnoise, maketheory = maketheory, parm=parm, info=info, silent = True, changeunits=changeunits)
        if k == 0:
            g = np.shape(map)[0]
            new_cube = np.zeros(g,g,zlim)
        new_cube[:,:,k] = map
        print 'smoothed slice', k, 'of', zlim #
    return cube, info
#-------------------------------------------------------------------------------------------
def makeobservable(map, addnoise=False, changeunits=False, info='',silent=False, units_in_photon=False):
    factor = (pix_size*1e3)**2 * flux_ratio * exptime * el_per_phot / gain #pix_size is in kpc
...
def makenoisy(data, factor=None, silent=False):
    ...
    if factor is None:
        factor = (pix_size*1e3)**2 * flux_ratio * exptime * el_per_phot / (planck * nu)
    data *= factor
...
if args.ppv:
    ...
    if args.X is not None:
        ...
        X = int(galsize/res_phys/2) - 1 #p-p values at which point to extract spectrum from the ppv cube
    if args.plotspec: print 'X position at which spectrum to be plotted=', X

    if args.Y is not None:
        Y = float(args.Y)
    else:
        Y = int(galsize/res_phys/2) - 1 #p-p values at which point to extract spectrum from the ppv cube
...
for i, Om in enumerate(Om_ar):
    ...
    if args.addbpt: 
        ...
        bpt_pixelwise(s, Om, res, res_phys, saveplot = args.saveplot, smooth=args.smooth, parm = parm, ker = ker, addnoise=args.addnoise, maketheory=args.maketheory)
    elif args.met: 
        metallicity(s, Om, res, res_phys, getmap=args.getmap, hide=args.hide, saveplot = args.saveplot, cmin=cmin, cmax=cmax, smooth=args.smooth, parm = parm, ker = ker, addnoise=args.addnoise, maketheory=args.maketheory)
    elif args.map: 
        map = emissionmap(s, Om, res, res_phys, line, saveplot = args.saveplot, cmin=cmin, cmax=cmax, off = off, smooth=args.smooth, \
        ...
        SFRmap_real, SFRmapQ0 = SFRmaps(s, Om, res, res_phys, getmap=args.getmap, cmin=cmin, cmax=cmax, off = off, \
        ...
        ppvcube = spec(s, Om, res, res_phys, col_ar[i], wmin=wmin, wmax=wmax, cmin=cmin, cmax=cmax, off = off, \
