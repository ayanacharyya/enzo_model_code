...
def readtargetlines(targetfile):
    target = []
    ...
    return lines
#-------------------------------------------------------------------------------------------
def anim_cube(cmin, cmax, galsize=26.0, pause=1, mapcube=False):   
    ...
    if mapcube: lines = readtargetlines('targetlines.txt')
    ...
    anim = animation.FuncAnimation(fig, animate, frames=lim, interval=pause, blit=False, init_func=init) #interval in milliseconds
    anim_running = True  
    print 'Animation running..Click to pause.'  
...
for arg in arg_arr:
    plt.pause(3)
    #fn = fn_prefix+'_DD0600_lgfOm=0.5,res=0.4kpc_927.02-6782.674A_specsmeared_'+str(arg)+'kmps_smeared_moff_parm5,4.7,51_noisy_obs'
    fn = fn_prefix+'_DD0600_lgfOm=0.5,res=0.4kpc_6400.0-6782.674A_specsmeared_'+str(arg)+'kmps_smeared_moff_parm5,4.7,51_noisy_obs'
    ...
    if args.plotratio: 
        plot_ratiomap(ppvcube, numlabels, denlabels, cmin, cmax, galsize=galsize, keep=args.keep, arg='for vres '+str(arg))
    elif args.plotmap:
        labels = readtargetlines('targetlines.txt')
        po.emissionmap(ppvcube, labels, line, cmin=cmin, cmax=cmax, fitsname=fn)
    elif args.plotspec:
        ...
        w, dummy2, dummy3, new_w, dummy4, wlist, llist, dummy7 = po.get_disp_array(100, 15, float(arg), 100, wmin=wmin, wmax=wmax, spec_smear=True)
        dispsol = new_w[1:]
        #po.spec_at_point(ppvcube, dispsol, wlist, llist, X, Y, wmin=wmin ,wmax=wmax, filename=fn)
        dummy1, dummy2 = fp.fitspaxel(ppvcube, dispsol, wlist, llist, X, Y, arg, wmin=wmin ,wmax=wmax, filename=fn)
    elif not args.noanim:
        anim_cube(cmin, cmax, galsize=galsize, pause=pause, mapcube=args.mapcube)       
print 'Finished!'