...
if __name__ == '__main__':
    ...
    for k in range(core_start, core_end+1):
        ...
        if (np.array(flam) > 0.).any():
            ...
            try:
                ...
                if args.contsub:
                    ...
                    flam_u = np.sqrt(
                        (flam_u / cont) ** 2 + (flam * cont_u / (cont ** 2)) ** 2)  # flam_u = dimensionless
                    flam /= cont  # flam = dimensionless
            except:
                print_mpi(
                    'parallel_fitting: line130: continuum fitting failed for pixel %d,%d: setting flam to zero.\n' % (
                    i, j), args)
        ...
        if args.debug:
            ...
            print_mpi('Deb110: flam in ergs/s/pc^2/A: Median, stdev, max, min= ' + str(np.median(flam)) + ',' + str(
                np.std(flam)) + ',' + \
    ...
    print_master(
        'Parallely: time taken for fitting of ' + str(ncells) + ' cells with ' + str(ncores) + ' cores= ' + str(
            t_diff / 60.) + ' min', args)
