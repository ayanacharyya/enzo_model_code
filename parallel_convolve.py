...
if __name__ == '__main__':
    ...
    convolved_cube = np.zeros((int(args.galsize/logbook.final_pix_size), int(args.galsize/logbook.final_pix_size), np.shape(cube)[2])) ###only if re-sampling convolved cube back to original resolution
    ...
    for k in range(core_start, core_end):
        ...
        result[np.log10(np.abs(median)) - np.log10(np.abs(result)) > 10.] = 0. #discarding any resulting pixel that is more than O(10) lower than input data, to avoid round off error pixels
        result = po.rebin(result, (np.shape(convolved_cube)[0], np.shape(convolved_cube)[1])) ###only if re-sampling convolved cube back to original resolution
