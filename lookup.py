...
def calc_Z(x, y, logOHcen, logOHgrad):
    ...
    r = np.sqrt((x - galsize / 2 - 2) ** 2 + (
            y - galsize / 2 - 2) ** 2)  # calculate galactocentric distance (kpc), with the galactic center being at an offset of (2,2)kpc w.r.t center of FoV
    Z = 10 ** (
            logOHcen - logOHsun + logOHgrad * r)  # assuming central met logOHcen and gradient logOHgrad dex/kpc, logOHsun = 8.77 (Dopita 16)
    return Z
...
for logOHgrad in logOHgrad_arr:
    ...
    for Om in Om_ar:
        ...
        Zout = 10 ** (np.log10(np.divide(fluxes[41], (fluxes[43] + fluxes[42]))) + 0.264 * np.log10(
            np.divide(fluxes[41], fluxes[40])))  # D16 method
        ...
        plt.figure()
        plt.plot(np.arange(galsize / 2), np.poly1d((logOHgrad, logOHcen))(np.arange(galsize / 2)) - logOHsun, c='brown',
                 ls='dotted', label='Target input gradslope=' + str('%.4F' % logOHgrad))  #
        ...
        for name in lines:
            head += '  ' + name
        outarr = np.row_stack((ind, x, y, vz, Z, age, m, n, U, np.log10(Q_H0), lpok, np.divide(Rs, 3.086e16),
                               np.divide(ri, 3.086e16), np.divide(r_m, 3.086e16), fluxes))
